...
def behaviorTracking(ser):
    # ser = serial.Serial('COM9', 115200)
    # Serial Arm Controller object should already have a connection
    ...
    while cap.isOpened(): 
        ...
        if results.multi_face_landmarks:
            ...
            mouth_3d_projection, jacobian = cv2.projectPoints(mouth_3d, rot_vec, trans_vec, cam_matrix, dist_matrix)
            ...
            headRotationAngle = int(np.arctan2(nose2Mouth2[1] - nose2Mouth1[1], nose2Mouth2[0] - nose2Mouth1[0]) * 180 / np.pi)
            headRotationAngle = int(map_range(headRotationAngle, 0, 180, 180, 0))
            headRotationAngle = int(process_running_average(headRotationRunning, headRotationAngle))
            headTiltAngle = int(map_range(x, -20, 20, 120, 0))
            ...
            outputStr = '090' + tripleDigitInttoString(torsoRotAngle) + tripleDigitInttoString(headRotationAngle) + tripleDigitInttoString(headTiltAngle) + '\n'
            # print(outputStr)
            # print("Sending data to serial arm controller")
            ser.send(bytes(outputStr, 'utf-8'))
            ...
            cv2.putText(image, "HeadRot: " + str(headRotationAngle), (500,200), cv2.FONT_HERSHEY_SIMPLEX, .5, (0,0,255), 2)
            cv2.putText(image, "HeadTilt: " + str(headTiltAngle), (500,250), cv2.FONT_HERSHEY_SIMPLEX, .5, (0,0,255), 2)
            cv2.putText(image, "TorsoRot: " + str(torsoRotAngle), (500,300), cv2.FONT_HERSHEY_SIMPLEX, .5, (0,0,255), 2)
            ...
            fps = 1 / totalTime
            # print("FPS: ", fps)
            cv2.putText(image, "FPS: " + str(np.round(fps,2)), (500,350), cv2.FONT_HERSHEY_SIMPLEX, .5, (0,0,255), 2)
